...
async function scheduleNextDayNotification(): Promise<Date> {
  ...
  console.log(`[Notifications] Next daily notification scheduled for: ${scheduledTime.toLocaleString()}`);
  return scheduledTime;
}
...
export async function notifyFriendPosted(posterUserId: string, posterName: string): Promise<number> {
  if (!config.vapidPublicKey || !config.vapidPrivateKey) {
    return 0;
  }
  ...
  const subscriptions = await db
    ...
    .where(
      and(
        ne(pushSubscriptions.userId, posterUserId),
        eq(notificationPreferences.friendPosts, true)
      )
    );
